...
!!$  call handle_errmsg(errstring, subname="wv_sat_methods_init")
  ...
  if (status /= 0) then 
     write(*,*) 'wv_sat_init :: ERROR allocating saturation vapor pressure table'
     call abort()
  end if
...
subroutine wv_sat_final
  ...
  if (allocated(estbl)) then
     ...
     if (status /= 0) then
        write(*,*) 'wv_sat_final :: ERROR deallocating table'
        call abort()
     end if
...
subroutine findsp_vc(q, t, p, use_ice, tsp, qsp)
  ...
  do i = 1,n
     if (status(i) == 2) then
        ...
        write(*,*) 'wv_saturation::FINDSP -- not converging'
        call abort()
     else if (status(i) == 8) then
        write(iulog,*) ' the enthalpy is not conserved at i = ', i
        write(iulog,*) ' t, q, p ', t(i), q(i), p(i)
        write(iulog,*) ' tsp, qsp ', tsp(i), qsp(i)
        write(*,*) 'wv_saturation::FINDSP -- enthalpy is not conserved'
        call abort()
     endif
  end do
